import type { Invoice, CreateInvoiceRequest, ApiResponse, InvoiceResponse } from "@/types/invoice";
...
export async function getInvoices(filters?: {
  ...
  if (filters?.tokenAddress) params.append("token_address", filters.tokenAddress);

  const url = `${API_URL}/invoice${params.toString() ? `?${params.toString()}` : ""}`;
 ...
 */
export async function createInvoiceInAPI(invoice: CreateInvoiceRequest): Promise<InvoiceResponse> {
  const response = await fetch(`${API_URL}/invoice`, {
    ...
    body: JSON.stringify({
      ...invoice,
      amount: typeof invoice.amount === "bigint" ? invoice.amount.toString() : invoice.amount,
    }),
  });
